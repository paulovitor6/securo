...
async def _create_details(
    session: AsyncSession, workspace_id: uuid.UUID, user_id: uuid.UUID, data: LoanDetailsCreate
) -> LoanDetails:
    """Create the loan and its schedule, handing back the row itself — the
    import needs the model to keep matching later CSV rows by name."""
    details = LoanDetails(
        user_id=user_id,
...
async def create_loan(
    ...
    details = await _create_details(session, workspace_id, user_id, data)
    return await _build_summary(session, details)
...
async def import_loans_csv(
    ...
    for i, row in enumerate(reader, start=2):
        ...
        if existing is None:
            loans_by_name[key] = await _create_details(session, workspace_id, user_id, data)
            created += 1
        else:
